...
// PPS4 ROM Reader
//
// Reads A05 type PPS4/2 ROMs. These are -17V supply ROMs
// with data clocked out by a two phse clock arrangement. ROM and
// RAM memory spaces exist and the ROM can be mask programmed to respond
// to either memory space. It can also respond to any of the AS select lines
// which are effectively extra address lines. So, dump the entire space
// and work out where the data is later.
// ROM data is 8 bits and RAM data is 4 bits. RAM data can be addressed
// using the A11X signal as an upper/lower nibble select, so RAM data
// needs a slightly different addressing mode.
// 
//
...
#define NUM_SCANS    1
// 16 AS areas or 1024 bytes, A11X 2 areas and RAM and ROM data
#define SLOT_SIZE    (16*1024*2*2)
...
const int  AB1_PIN    =  8;
const int  AB2_PIN    =  9;
const int  AB3_PIN    = 10;
const int  AB4_PIN    = 11;
const int  AB5_PIN    = 12;
const int  AB6_PIN    = 13;
const int  AB7_PIN    = 14;
const int  AB8_PIN    = 15;
const int  AB9_PIN    = 16;
const int  AB10_PIN   = 17;
const int  AB11X_PIN  = 18;

const int  CLKA_PIN    = 19;
const int  CLKB_PIN    = 20;

const int  AS0_PIN    = 21;
const int  AS1_PIN    = 22;
const int  AS2_PIN    = 26;
const int  AS3_PIN    = 27;
  ...
  for(int i=0; i<NUM_AS; i++)
    {
      if( as & (1<<i) )
	{
	  gpio_put(as_pins[i], 1);
	}
      else
	{
	  gpio_put(as_pins[i], 0);
	}
    }
}
...
void usb_dump(void)
{
  // We scan the device with all permutations of AS and AB signals and also with
  // A11X in both states. This should result in data being returned however the
  // device has been mask programmed.
...
#define LINELEN 32
  ...
  // Dump over the entire address range, with every permutation of AS signals as well

  // Set the clock to the initial states
  ...
  for(as=0; as<16; as++)
      ...
      for(addr=0; addr < (1<<NUM_ADDR); addr++)
	{
	  ...
	  // Set the address lines up
	  // We use the same address for RAM and ROM spaces
	  set_address(addr);
	  sleep_us(100);

	  // We have to clock the data out with the two phase clock
	  // We interleave the RAM and ROM data for ease of dumping

	  // Move to clear bus clock cycle
	  gpio_put(CLKA_PIN, 0);
	  sleep_us(10);
	  gpio_put(CLKB_PIN, 1);
	  sleep_us(10);

	  // Move to RAM readout cycle
	  gpio_put(CLKB_PIN, 0);
	  sleep_us(10);
...
int main()
{
  ...
  for (int i=0; i<NUM_AS; i++)
    {
      // Set up directions for the control lines
      gpio_init(as_pins[i]);
      gpio_set_dir(as_pins[i], GPIO_OUT);
    }
  ...
  // Clock lines are outputs
  gpio_init(CLKA_PIN);
  gpio_set_dir(CLKA_PIN, GPIO_OUT);
  gpio_init(CLKB_PIN);
  gpio_set_dir(CLKB_PIN, GPIO_OUT);
